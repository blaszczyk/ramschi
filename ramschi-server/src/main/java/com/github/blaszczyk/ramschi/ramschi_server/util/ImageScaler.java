...
public class ImageScaler {
    ...
    public void addThumbnailAndPreview(ImageEntity entity) {
        try {
            final BufferedImage badImage = ImageIO.read(new ByteArrayInputStream(entity.getOriginal()));
            final Metadata metadata = ImageMetadataReader.readMetadata(new ByteArrayInputStream(entity.getOriginal()));
            ...
            System.out.println();
            final BufferedImage image = switch (orientation) {
                case 1 -> badImage;
    ...
    private byte[] scale(BufferedImage image, int width, int height) throws IOException {
